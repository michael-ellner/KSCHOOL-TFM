...
def word_count_tweets_mongo(query='Obamacare', lang='en', word_num=50, scale='linear', hourP=10000000):
    """
    ...
    print("mongo petition {} {} {}".format(query,lang,hourP))
    t1=datetime.utcnow()
    tweets_text = [tweet['text'] for tweet in collection.find({"tag": query, "lang" : lang, "dateTime": {"$gte" : datetime.utcnow() - timedelta(hours=hourP)}}, projection={'text':True,'_id':False}).limit(100)]
    print("mongo petition {} {} {} finished in {}".format(query,lang,hourP,t1-datetime.utcnow()))
    ...
    #####
    if scale == "sqrt":
        dfJ['size']=dfJ['size'].apply(numpy.sqrt) / float(dfJ['size'].sum())*150
    elif scale=="log":
        dfJ['size']=dfJ['size'].apply(numpy.log) / float(dfJ['size'].sum())*1000
    else:
        dfJ['size']=dfJ['size'] / float(dfJ['size'].sum())*100
  
    return  dfJ.to_dict(orient='record ')
    #return  json.dumps(dfJ.to_dict(orient='record '))
